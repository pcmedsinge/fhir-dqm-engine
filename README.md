...
**v0.3.0-alpha.1 — First CQL measure running end-to-end.**

Phase 3 is live: **CMS165 Controlling High Blood Pressure (CBP)** executes against 279 HAPI patients using NCQA's JavaScript CQL engine (`cql-execution` + `cql-exec-fhir`). The engine produces a FHIR `MeasureReport`, persists it to HAPI, and exposes a care-gap list. New endpoints: `POST /v1/measures/:id/compute`, `GET /v1/measures/:id/report`, `GET /v1/measures/:id/gaps`, `GET /v1/measures`.

Phase 2 foundation: HAPI FHIR R4 server (local dev via Docker), 279 synthetic Synthea patients, bulk loader with idempotency, NestJS FHIR client module, `/health` FHIR indicator, `/v1/fhir/stats` endpoint.
...
# {"fhirServerUrl":"http://localhost:8080/fhir","fhirVersion":"4.0.1","resourceCounts":{"Patient":279,...}}

# Compute a quality measure (Phase 3+)
curl -X POST http://localhost:3000/v1/measures/cms165-cbp/compute \
  -H "Content-Type: application/json" \
  -d '{"periodStart":"2026-01-01","periodEnd":"2026-12-31"}'
# Returns a FHIR MeasureReport with IPP/denominator/numerator counts and a measure score
```
...
| CQL execution            | `cql-execution` + `cql-exec-fhir` (JavaScript / NCQA)               | Ships TypeScript types; runs in-process; no separate JVM container        |
